...
- [Part 1 — The Alerting Pipeline](#part-1--the-alerting-pipeline)
- [Part 2 — Tuning Suricata for an 8GB Box](#part-2--tuning-suricata-for-an-8gb-box)
- [Part 3 — HOME_NET, Directionality, and Getting Auto-Banned](#part-3--home_net-directionality-and-getting-auto-banned)
- [Part 4 — Attack Demo: Recon to Root](#part-4--attack-demo-recon-to-root)
- [Part 5 — What the Alerts Actually Prove](#part-5--what-the-alerts-actually-prove)
...
![Zenmap topology of the VLAN 60 segment](media/topology.png)
*Zenmap topology: localhost (Kali), OPNsense as the gateway, and the target host 192.168.60.146.*
...
![Pushover notification stack](media/notifications-scan.png)
*Recon in progress: a stack of ET SCAN alerts — Nmap Scripting Engine, SSH scan, OS-detection probe, and suspicious inbound probes to the SQL/VNC/RDP ports as Kali enumerates the target.*
...
![CrowdSec decisions list showing the Kali ban and the allowlist fix](media/crowdsec-ban.png)
...
This lit up a stack of **ET SCAN** alerts — Nmap Scripting Engine user-agent, potential SSH scan, OS-detection probe, and "suspicious inbound" hits on the MySQL/MSSQL/Oracle/PostgreSQL/VNC/RDP ports as nmap walked the services. Recon is the loudest, most reliably detected stage of any attack, and the notification stack shows it clearly.

![ET SCAN notification stack during recon](media/notifications-scan2.png)
...
![Nikto scan output](media/nikto.png)
![Web-attack notifications from nikto](media/notifications-web.png)

*(See [Part 5](#part-5--what-the-alerts-actually-prove) for the important caveat on the CVE-named web alerts.)*
...
![Samba exploit landing a root shell](media/samba-root.png)

### The detection that ties it together

Here's the best part. When I ran `id` inside the root shell, Suricata caught the response **coming back from the victim**:

```
Suricata: Potentially Bad Traffic
GPL ATTACK_RESPONSE id check returned root
192.168.60.146 -> 192.168.10.57:38873 (sev 2)
```

![id check returned root alert](media/notifications-root.png)
...
### The mid-attack lesson: reverse shell vs. bind shell
...
During the nikto stage, my phone showed alerts naming CVEs like **VMware Spring Cloud Directory Traversal (CVE-2020-5410)**. Impressive-looking — but Metasploitable is an old Linux box and doesn't run that software. Those alerts fired because Suricata **pattern-matched the malicious requests** nikto sent, not because anything was exploited. They are **detection events**: proof the IDS flags exploit *attempts*, not proof of exploitation.

A sharp example of this lives right in my own alerts: **two different Samba CVEs showed up, and only one was my actual attack path.**

- The SMB traffic tripped **`ET EXPLOIT Samba Arbitrary Module Loading (CVE-2017-7494)`** — "SambaCry." That's the signature that matched on port 445, and it's the flashy, recognizable CVE in the screenshot.
- But the shell I actually landed was via **CVE-2007-2447** (username map script command injection) — a *different, older* Samba bug.

So the impressive-looking CVE-2017-7494 alert is a **detection** of Samba exploit activity; the **exploitation** that gave me root was CVE-2007-2447. Same service, two different CVEs, and only reading the details tells them apart.

| Event | What it proves |
|---|---|
| ET SCAN stack (nmap, SSH/VNC/SQL probes) | IDS detects reconnaissance — real and accurate |
| CVE-named web alerts from nikto (e.g. CVE-2020-5410) | IDS detects exploit **attempts** by signature (detection yes; exploitation no — target isn't that software) |
...
1. **Detection ≠ exploitation.** An alert proves an attempt was *seen*, not that it *worked* — except when it does: the `id check returned root` response is the IDS catching a real compromise. Reading direction and signature is what tells them apart.
2. **Reputation feeds are dead weight in an isolated lab** — trimming them fixed my memory-driven crashes. Match the ruleset to the actual threat surface.
3. **Rule reloads are the memory spike.** Fewer rules = smaller spike = stable IDS on modest hardware.
4. **Suricata rules are directional.** `HOME_NET` / `EXTERNAL_NET` placement determines whether attack rules fire at all.
5. **Segmentation cuts both ways.** It contains attackers *and* breaks naive reverse shells — understanding the return path is what lets you adapt (bind vs. reverse).
6. **Your own defenses will bite you** — CrowdSec auto-banned my attack box for port scanning. Whitelisting your tooling is a real step, and the ban proves the control works.
7. **Build the feedback loop.** Real-time alerts on my phone turned "logs I never read" into "I know the instant something happens."
...
All images referenced above live in a `media/` folder at the repo root. Rename your files to exactly these names so the embeds resolve.

| Rename your file to… | This is which screenshot | Used in |
|---|---|---|
| `topology.png` | Zenmap topology — 4 hosts, OPNsense in the middle, target 60.146 | Lab Architecture |
| `notifications-scan.png` | ET SCAN stack — nmap NSE, SSH scan, OS probe, SQL/VNC port probes | Part 1 |
| `notifications-scan2.png` | Second ET SCAN stack (the MySQL/MSSQL/Oracle/PostgreSQL inbound probes) | Part 4, Stage 1 |
| `notifications-samba-cve.png` | **The hero shot** — `ET EXPLOIT Samba … CVE-2017-7494` + CrowdSec bans in one frame | Part 4/Part 5 (and LinkedIn lead) |
| `notifications-root.png` | Close-up of `GPL ATTACK_RESPONSE id check returned root` (60.146 → 10.57) | Part 4, "detection that ties it together" |
| `notifications-web.png` | Web-attack stack — CodeRed root.exe, LFI, /~root, iisadmin | Part 4, Stage 2 |
| `crowdsec-ban.png` | `cscli decisions list` — Kali banned for pf-scan + the allowlist fix | Part 3 |
| `nikto.png` | Nikto scan terminal output | Part 4, Stage 2 |
| `samba-root.png` | msfconsole — bind_netcat exploit → `whoami: root` | Part 4, Stage 3 |
| `samba-root.mp4` | *(optional)* screen recording of the exploit landing | Part 4, Stage 3 |

> **Matching your actual files:** the CVE-2017-7494 + CrowdSec-ban screenshot is `notifications-samba-cve.png` (your best one). The clean recon stacks are the `notifications-scan*.png` pair. The `id check returned root` close-up is `notifications-root.png`.

### How to upload to GitHub

**Easiest (browser, no git needed):**
1. Create a new repo on github.com (e.g. `suricata-pushover-ids-lab`).
2. On the repo's main page, **Add file → Upload files**, and drag in your renamed `README.md`.
3. Create the media folder in the same upload: drag your images in, but first put them in a folder named `media` on your computer, then drag that whole folder onto the upload page. (GitHub's web uploader keeps the folder structure.)
4. Commit. The README renders on the repo homepage automatically, images and all.

**For the videos (`.mp4`):** GitHub won't embed an mp4 via markdown `![]()`. Instead, open the README in the github.com editor, position your cursor where you want it, and **drag the mp4 straight into the editor** — GitHub uploads it and inserts a playable embed automatically. Do this after the initial upload.

**If you prefer git CLI:**
```bash
mkdir suricata-pushover-ids-lab && cd suricata-pushover-ids-lab
mkdir media
# copy README.md here, and your renamed images into media/
git init
git add .
git commit -m "IDS alerting pipeline writeup + lab evidence"
git branch -M main
git remote add origin https://github.com/<you>/suricata-pushover-ids-lab.git
git push -u origin main
```

### Before you make the repo public
- **Scrub the notifier script** if you include it: replace the hardcoded Pushover token/user key with `YOUR_APP_TOKEN` / `YOUR_USER_KEY`, and **regenerate those keys** in the Pushover dashboard (they've existed in plaintext, so rotate them regardless).
- Private lab IPs (`192.168.x.x`) are fine to leave — they're non-routable and reveal nothing.
